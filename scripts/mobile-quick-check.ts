...
(async () => {
  ...
  await page.screenshot({ path: path.join(OUT, 'check-sound-prompt.png') });
  ...
  for (const pct of [0.05, 0.3, 0.6, 0.95]) {
    await page.evaluate((y) => window.scrollTo(0, y), Math.floor(totalH * pct));
    await page.waitForTimeout(1500);
    await page.screenshot({ path: path.join(OUT, `check-scroll-${Math.round(pct * 100)}.png`) });
  }
