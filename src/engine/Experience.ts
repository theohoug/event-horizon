...
export class Experience {
  ...
  private profileGpu(renderer: THREE.WebGLRenderer): PerfConfig {
    ...
    try {
      const data = JSON.parse(localStorage.getItem('eh_perf_v16') || '{}');
      if (data.fp === fingerprint) return data.cfg as PerfConfig;
    } catch {}
    for (let v = 6; v <= 15; v++) { try { localStorage.removeItem(`eh_perf_v${v}`); } catch {} }
...
gl_FragColor=vec4(col,1.0);}`;
    ...
    try { localStorage.setItem('eh_perf_v16', JSON.stringify({ fp: fingerprint, cfg: config })); } catch {}
    return config;

    } catch {
      return { dpr: 1.0, maxSteps: 48, qualityMedium: true, gpgpuTexSize: 128, starfieldCount: 6000, bloomPasses: 2, bloomScale: 0.2, motionBlur: false, antialias: false, gpuScore: 15, quality: 'medium' as const };
    }
  }
  ...
  private async init() {
    ...
    this.canvas.addEventListener('webglcontextlost', (e) => {
      e.preventDefault();
      try { localStorage.removeItem('eh_perf_v16'); } catch {}
    });
  ...
  private animate(timestamp?: number) {
    ...
    if (this.emergencySlowFrames >= 2) {
      ...
      ScrollTrigger.update();
      const skipLevels = this.emergencySlowFrames >= 3 ? 4 : 2;
